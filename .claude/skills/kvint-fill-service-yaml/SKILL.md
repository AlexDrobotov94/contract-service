...
### embeds (только для type: website)

Если тип компонента — `website`:

«Этот сайт является shell-приложением и встраивает другие микрофронты? (да/нет)»

Если да — покажи список зарегистрированных serviceRef из `services.schema.json#/definitions/serviceRef/enum`.

Для каждой записи: **ID встраиваемого MFE** — выбор из списка.

После каждой: «Добавить ещё? (да/нет)»
...
- `consumesApis` — `[]`
- `embeds` — не включай если тип не `website` или пользователь не указал
